// ============================================================
// 函数名称: sub_628630
// 起始地址: 0x628630
// 备注: 带有 ; => 的注释为 Binary Ninja 解析出的高级语义 Token
// ============================================================

00628630    mov eax, dword ptr ds:[0x008DB664]
00628635    mov edx, dword ptr ds:[0x008DB670]              ; => [ Data: data_8db670 ]
0062863B    add eax, 0xFFFFF830
00628640    cmp eax, 0x10
00628643    jnbe 0x006288FB
00628649    jmp dword ptr ds:[eax*4+0x628900]               ; => [ Data: data_8db664 ]
00628650    cmp edx, 0x08
00628653    jnbe 0x00628698
00628655    jmp dword ptr ds:[edx*4+0x628944]
0062865C    mov dword ptr ds:[ecx], 0x86E998                ; => [ Data: data_86e998 ]
00628662    mov eax, 0x11
00628667    ret
00628668    mov dword ptr ds:[ecx], 0x86EA20                ; => [ Data: data_86ea20 ]
0062866E    mov eax, 0x03
00628673    ret
00628674    mov dword ptr ds:[ecx], 0x86E938                ; => [ Data: data_86e938 ]
0062867A    mov eax, 0x0A
0062867F    ret
00628680    mov dword ptr ds:[ecx], 0x86E988                ; => [ Data: data_86e988 ]
00628686    mov eax, 0x02
0062868B    ret
0062868C    mov dword ptr ds:[ecx], 0x86E8E8                ; => [ Data: data_86e8e8 ]
00628692    mov eax, 0x06
00628697    ret
00628698    test edx, edx
0062869A    jnz 0x006288FB
006286A0    mov dword ptr ds:[ecx], 0x86E918                ; => [ Data: data_86e918 ]
006286A6    lea eax, ds:[edx+0x04]
006286A9    ret
006286AA    sub edx, 0x00
006286AD    jz 0x006286C2
006286AF    sub edx, 0x01
006286B2    jnz 0x006288FB
006286B8    mov dword ptr ds:[ecx], 0x86E8D8                ; => [ Data: data_86e8d8 ]
006286BE    lea eax, ds:[edx+0x02]
006286C1    ret
006286C2    mov dword ptr ds:[ecx], 0x86E8C8                ; => [ Data: data_86e8c8 ]
006286C8    mov eax, 0x02
006286CD    ret
006286CE    sub edx, 0x00
006286D1    jz 0x006286E6
006286D3    sub edx, 0x01
006286D6    jnz 0x006288FB
006286DC    mov dword ptr ds:[ecx], 0x86E8B8                ; => [ Data: data_86e8b8 ]
006286E2    lea eax, ds:[edx+0x02]
006286E5    ret
006286E6    mov dword ptr ds:[ecx], 0x86E8A8                ; => [ Data: data_86e8a8 ]
006286EC    mov eax, 0x02
006286F1    ret
006286F2    test edx, edx
006286F4    jnz 0x006288FB
006286FA    mov dword ptr ds:[ecx], 0x86E868                ; => [ Data: data_86e868 ]
00628700    lea eax, ds:[edx+0x02]
00628703    ret
00628704    test edx, edx
00628706    jnz 0x006288FB
0062870C    mov dword ptr ds:[ecx], 0x86E878                ; => [ Data: data_86e878 ]
00628712    lea eax, ds:[edx+0x06]
00628715    ret
00628716    test edx, edx
00628718    jnz 0x006288FB
0062871E    mov dword ptr ds:[ecx], 0x86E830                ; => [ Data: data_86e830 ]
00628724    lea eax, ds:[edx+0x01]
00628727    ret
00628728    sub edx, 0x00
0062872B    jz 0x00628740
0062872D    sub edx, 0x01
00628730    jnz 0x006288FB
00628736    mov dword ptr ds:[ecx], 0x86E7A8                ; => [ Data: data_86e7a8 ]
0062873C    lea eax, ds:[edx+0x0F]
0062873F    ret
00628740    mov dword ptr ds:[ecx], 0x86E838                ; => [ Data: data_86e838 ]
00628746    mov eax, 0x06
0062874B    ret
0062874C    test edx, edx
0062874E    jnz 0x006288FB
00628754    mov dword ptr ds:[ecx], 0x86E820                ; => [ Data: data_86e820 ]
0062875A    lea eax, ds:[edx+0x02]
0062875D    ret
0062875E    cmp edx, 0x03
00628761    jnbe 0x006288FB
00628767    jmp dword ptr ds:[edx*4+0x628968]
0062876E    mov dword ptr ds:[ecx], 0x86E788                ; => [ Data: data_86e788 ]
00628774    mov eax, 0x01
00628779    ret
0062877A    mov dword ptr ds:[ecx], 0x86E790                ; => [ Data: data_86e790 ]
00628780    mov eax, 0x03
00628785    ret
0062879E    cmp edx, 0x04
006287A1    jnbe 0x006288FB
006287A7    jmp dword ptr ds:[edx*4+0x628978]
006287AE    mov dword ptr ds:[ecx], 0x86E6C8                ; => [ Data: data_86e6c8 ]
006287B4    mov eax, 0x01
006287B9    ret
006287BA    mov dword ptr ds:[ecx], 0x86E6D0                ; => [ Data: data_86e6d0 ]
006287C0    mov eax, 0x0A
006287C5    ret
006287C6    mov dword ptr ds:[ecx], 0x86E670                ; => [ Data: data_86e670 ]
006287CC    mov eax, 0x08
006287D1    ret
006287D2    mov dword ptr ds:[ecx], 0x86E6B0                ; => [ Data: data_86e6b0 ]
006287D8    mov eax, 0x03
006287DD    ret
006287DE    mov dword ptr ds:[ecx], 0x86E5F0                ; => [ Data: data_86e5f0 ]
006287E4    mov eax, 0x03
006287E9    ret
